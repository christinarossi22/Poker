...
public class gradePoker {
	
	private boolean debug = true;		//DEBUG VARIABLE - SET TO FALSE TO HAVE PRODUCTION (i.e. SLOW) DEALING
	private int timeout = 10;			//AMOUNT OF TIME TO WAIT BEFORE MOVING ON...in milliseconds (1000 = one second)
	
		

	private int button = 0;				//WHO IS THE DEALER
	private int raiseCounter = 0;		//HOW MANY TIMES HAS THERE BEEN A RAISE IN THIS ROUND OF BETTING
	private final int raiseLimit = 4;	//MAX NUM OF TIMES SOMEONE CAN RAISE PER ROUND
	private final int initStack = 1000;	//STARTING STACK AMOUNT - USUALLY 100
	private boolean itsOn = false;		//IF THE GAME IS STILL ON (true) OR OVER (false)
	private Card[] deck = null;			//GAME DECK
	private Card[] board = new Card[5]; //BOARD, OR DEALT CARDS		
	private int pot = 0;
	private final int smallBlind = 5;
	private final int bigBlind = 10;
	private int topCard = 0;
	private boolean allIn = false;		//SET IF SOMEONE MAKES ALL IN CALL
	private int[] stacks = {initStack, initStack};
	private Card[] buttonHand = new Card[2];
	private Card[] nonHand = new Card[2];
	private Player[] players = new Player[2];
	private simplegraphics SG = new simplegraphics();
	private String[] args;
	
	public gradePoker() {
		this(new String[] {"gradeAI", "gradeAI"});
	}
	
	public gradePoker(String player) {
		this(new String[] {"gradeAI", player});
	}
	
	public gradePoker(String grader, String student) {
		this(new String[] {grader, student});
	}
	
	public gradePoker(String[] a) {
		args = a;
	}
			...
			while (itsOn) {
				...
				while (resp == "bet") {
					...
					if (resp == "call" && stacks[action] <= toYou) {
						...
						//System.out.println(stacks[0] + "|" + stacks[1] + "|" + pot);
					}
